...
This tutorial demonstrates how to analyse data from SILAC MS/MS experiment in *Proteus*. We strongly suggest the user should read the "unlabelled" tutorial first and familiarize themselves with the package.

```{r, eval=FALSE}
vignette("unlabelled", package="proteus")
```

Here, we are only going to point out the differences between unlabelled and SILAC analysis. For this, we use a small subset of data from [Ly et al. (2018)](https://wellcomeopenresearch.org/articles/3-51/). These data are distributed in a separate package *proteusSILAC*, which needs loading first:
...
Now we create protein data. The `aggregate.fun=aggregateMedian` indicates that protein ratios are medians of constituent peptide ratios. This approach is appropriate for SILAC data, unlike the default hi-flyer method which only works for unlabelled (or TMT) data.
