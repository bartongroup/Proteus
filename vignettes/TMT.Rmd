...
This tutorial demonstrates how to analyse data from TMT MS/MS experiment in *Proteus*. We strongly suggest the user should read the "unlabelled" tutorial first and familiarize themselves with the package. Here, we are only going to point out the differences between unlabelled and TMT analysis

```{r, eval=FALSE}
vignette("unlabelled", package="proteus")
```

Here we use example data distributed in a separate package *proteusTMT*, which needs loading first:
